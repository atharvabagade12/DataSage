    ...
    <div class="hero-section">
      <div class="hero-content">
        ...
        <div class="dataset-summary">
          <span class="summary-item"><strong>{{ fileData?.name || 'Dataset' }}</strong></span>
          <span class="summary-divider">•</span>
          <span class="summary-item">{{ dataset.length }} rows</span>
          <span class="summary-divider">•</span>
          <span class="summary-item">{{ availableColumns.length }} columns available</span>
        </div>
      </div>
    </div>
    ...
    <div class="main-content">
      ...
      <aside class="column-selector">
        <div class="selector-header">
          ...
          <p class="selector-subtitle">Select the column you want to predict or analyze</p>
        ...
        <div class="column-list-container">
          ...
          <div v-if="filteredColumns.length === 0" class="no-columns">
            ...
            <button @click="searchQuery = ''; activeFilter = 'all'" class="clear-filters-btn">
              Clear filters
            </button>
          </div>
            ...
            >
              <div class="column-info">
                ...
                <div class="column-meta">
                  ...
                  <span class="unique-count">{{ column.uniqueValues }} unique</span>
                  <span v-if="column.missingPercent > 0" class="missing-percent">
                ...
                <div v-if="column.suitabilityScore" class="suitability-score">
                  ...
                  <span class="score-text">{{ column.suitabilityScore }}% suitable</span>
                </div>
              </div>
            </div>
        ...
        <div class="selection-helper">
          <div v-if="!selectedColumn" class="helper-message">
            ...
            <span class="helper-text">Click a column above to start exploring</span>
          </div>
...
<main class="preview-panel">
  <div class="preview-header">
    <h3>
      {{ selectedColumn ? `Exploring: ${selectedColumn.name}` : "Select a column to preview" }}
    </h3>
    <div v-if="selectedColumn" class="chart-controls">
      ...
      <!-- ✅ ENCODING INDICATOR -->
      <div v-if="encodingInfo && encodingInfo.encoding !== 'none'" class="encoding-badge">
  ...
  <div class="chart-container">
    <div v-if="!selectedColumn" class="empty-state">
      ...
      <p>Select a column from the left panel to see its distribution and characteristics.</p>
    </div>
    ...
    <div v-else class="interactive-chart">
      ...
      <div v-if="!isLoadingChart" class="chart-insights">
        ...
        <div v-if="encodingInfo && encodingInfo.encoding !== 'none'" class="encoding-info">
          <h4>🔄 Data Encoding Applied</h4>
          <div class="encoding-details">
            <p><strong>Type:</strong> {{ encodingInfo.encoding | capitalize }}</p>
            <p><strong>Note:</strong> {{ encodingInfo.note }}</p>
            <div v-if="encodingInfo.mapping && Object.keys(encodingInfo.mapping).length <= 10" class="encoding-mapping">
              <h5>Value Mapping:</h5>
              <div class="mapping-grid">
                <div 
                  v-for="[original, encoded] in Object.entries(encodingInfo.mapping).slice(0, 8)" 
                  :key="original"
                  class="mapping-item"
                >
        ...
        <div v-else class="target-insights">
          ...
          <div class="insight-section">
            ...
            <div class="stats-grid">
              ...
              <div class="stat-item">
                ...
                <span class="stat-value">{{ selectedColumn.uniqueValues.toLocaleString() }}</span>
              </div>
              <div class="stat-item">
                <span class="stat-label">Missing Data</span>
                <span class="stat-value">{{ selectedColumn.missingPercent.toFixed(1) }}%</span>
              </div>
              <div v-if="selectedColumn.type === 'number'" class="stat-item">
                <span class="stat-label">Range</span>
                <span class="stat-value">{{ selectedColumn.min }} - {{ selectedColumn.max }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="insight-section">
            ...
            <div class="suitability-analysis">
              <div class="suitability-score-large">
                <div
                  class="score-circle"
                  :class="getScoreClass(selectedColumn.suitabilityScore || 0)"
                >
                  {{ selectedColumn.suitabilityScore || 0 }}%
                </div>
                <div class="score-description">
                  {{ getSuitabilityDescription(selectedColumn.suitabilityScore || 0) }}
                </div>
              </div>
              ...
              <div class="suitability-factors">
                <div
                  ...
                  class="factor-item"
                >
                  <span class="factor-icon" :class="factor.status">{{ factor.icon }}</span>
          ...
          <div class="insight-section">
            ...
            <div class="sample-values">
              <div
                v-for="(value, index) in getSampleValues()"
                :key="index"
                class="sample-item"
              >
    ...
    <div class="action-footer">
      <div class="footer-content">
        ...
        <div class="footer-actions">
          ...
          >
            <span>Continue to configure algorithm</span>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor">
              <path d="M8.59 16.59L10 18l6-6-6-6-1.41 1.41L13.17 12z" />
            </svg>
          </button>
...
const filteredColumns = computed(() => {
  ...
  if (activeFilter.value !== "all") {
    if (activeFilter.value === "recommended") {
      ...
      columnsToFilter = columnsToFilter.filter((col) => col.originalType === activeFilter.value);
    }
  }
...
const checkBackendConnection = async () => {
  try {
    const response = await fetch('http://localhost:8000/api/health', { timeout: 3000 })
    if (response.ok) {
      ...
      if (storedDatasetId) {
        ...
        console.log('📊 Backend dataset ID:', storedDatasetId)
      }
    } else {
...
const detectProblemTypeWithBackend = async (column) => {
  ...
  const cleanData = targetData.filter(val => val != null && val !== undefined && val !== '' && val !== 'null')
...
const loadPreviousData = async () => {
  try {
    console.log('📁 Loading optimized processed data...')
    ...
    console.log('📊 Loaded optimized data package:', {
    ...
    if (!data.backendDatasetId || !mlStore.backendConnected) {
      throw new Error('Backend not available or dataset not uploaded')
    }

    try {
      console.log('🔄 Fetching dataset page from backend...')
      
      // ✅ FETCH SAMPLE DATA FOR UI
      const pageResponse = await fetch(`http://localhost:8000/api/datasets/${data.backendDatasetId}/page?page=1&page_size=50`)
      
      if (pageResponse.ok) {
        const pageData = await pageResponse.json()
        console.log('✅ Backend dataset page loaded')
        
        // ✅ SET DATASET WITH SAMPLE DATA
        dataset.value = pageData.data || []
        
        // ✅ SET COLUMNS FROM BACKEND
        if (data.columns && data.columns.length > 0) {
          columns.value = data.columns.map(colName => {
            const dtype = data.dtypes?.[colName] || 'object'
            return {
              name: colName,
              type: dtype.includes('int') || dtype.includes('float') ? 'numeric' : 'categorical',
              unique: 0,
              missing: 0
            }
          })
        } else {
          // Fallback: extract from sample data
          if (pageData.data && pageData.data.length > 0) {
            columns.value = Object.keys(pageData.data[0]).map(colName => ({
              name: colName,
              type: 'categorical',
              unique: 0,
              missing: 0
        ...
        // ✅ SET DATASET INFO
        datasetInfo.value = {
          rowCount: data.totalRows || pageData.total_rows || 0,
          columnCount: data.totalColumns || columns.value.length
        }
        
        console.log('✅ Columns loaded:', columns.value.length)
        console.log('✅ Dataset info:', datasetInfo.value)
    ...
    alert('Failed to load dataset. Please go back and try again.')
    router.push('/data-preview')
  }
}
...
const processColumnsForTargetSelection = () => {
  ...
  if (!dataset.value || dataset.value.length === 0 || !columns.value || columns.value.length === 0) {
  ...
  availableColumns.value = columns.value.map(column => {
    ...
    const cleanData = rawData.filter(val => 
...
const calculateTargetSuitability = (analysis) => {
  ...
  if (analysis.name.includes('_') && !analysis.isEncoded) {
    console.log(`❌ Column with underscore ${analysis.name} - likely encoded feature`)
    score -= 40
  }
  ...
  if (analysis.missingPercent > 10) {
    ...
    console.log(`⚠️ High missing data ${analysis.name}: ${analysis.missingPercent}% - penalty: ${penalty}`)
    score -= penalty
  }
  ...
  if (!analysis.isEncoded) {
    ...
    if (analysis.originalType === 'categorical' || analysis.type === 'string') {
      ...
      console.log(`✅ Original categorical column ${analysis.name} - bonus +20`)
    }
    ...
    // Good multi-class classification (3-20 classes)
      ...
      score += 15
      console.log(`✅ Good cardinality ${analysis.name}: ${analysis.uniqueValues} classes - bonus +15`)
    }
    
    // Regression-friendly (many unique values)
    else if (analysis.uniqueValues > 20 && (analysis.type === 'number' || analysis.originalType === 'numerical')) {
      score += 20
      console.log(`✅ Regression-friendly ${analysis.name}: ${analysis.uniqueValues} unique values - bonus +20`)
    }
  ...
  if (analysis.type !== 'number' && analysis.originalType !== 'numerical') {
    ...
    if (uniqueRatio > 0.8) { // More than 80% unique values
      score -= 25
      console.log(`⚠️ Too many unique values ${analysis.name}: ${uniqueRatio.toFixed(2)} ratio - penalty -25`)
    }
  }

  // ❌ PENALTY: Likely feature engineering artifacts
  const featurePatterns = ['_mean', '_sum', '_count', '_std', '_min', '_max', '_encoded', '_transformed']
  if (featurePatterns.some(pattern => analysis.name.toLowerCase().includes(pattern))) {
    score -= 30
    console.log(`❌ Feature engineering artifact ${analysis.name} - penalty -30`)
  }
  ...
  console.log(`📊 Final suitability score for ${analysis.name}: ${finalScore}`, {
    isEncoded: analysis.isEncoded,
    encoding: analysis.encoding,
...
const getTargetRecommendations = () => {
  ...
  if (isIDColumn(column)) {
    return [{
      icon: '🚫',
      title: 'Not Suitable for Prediction',
      description: 'This appears to be an identifier column',
      confidence: 0,
      reasoning: 'ID columns cannot be meaningfully predicted'
    }]
  }
  
  const missingRatio = column.missingValues / column.totalRows
  ...
  if (column.type === 'number') {
    ...
    if (column.uniqueValues === 2) {
      recommendations.push({
        icon: '🎯',
        title: column.hasEncoding ? 'Binary Classification (Encoded)' : 'Binary Classification',
        description: 'Perfect for yes/no predictions',
        confidence: column.hasEncoding ? 96 : 93,
        algorithms: ['Logistic Regression', 'Random Forest', 'SVM'],
        reasoning: column.hasEncoding ? 
          `Smart encoding applied: ${column.encodingInfo.note}` : 
    ...
    else if (column.encoding === 'ordinal') {
      recommendations.push({
        icon: '📊',
        title: 'Ordinal Classification',
        description: 'Preserves natural ordering',
        confidence: 90,
        algorithms: ['Ordinal Regression', 'Random Forest'],
        reasoning: `Ordinal pattern detected: ${column.encodingInfo.note}`,
        estimatedAccuracy: '80-90%',
        taskType: 'ordinal_classification'
      })
    }
    
    // Multi-class classification
    else if (column.uniqueValues > 2 && column.uniqueValues <= 10) {
      recommendations.push({
        icon: '🎲',
        title: 'Multi-class Classification',
        description: `Classify into ${column.uniqueValues} categories`,
        confidence: 82,
        algorithms: ['Random Forest', 'XGBoost'],
        reasoning: 'Low cardinality suitable for classification',
    ...
    // Regression
    if (column.uniqueValues >= 20) {
      recommendations.push({
        icon: '📈',
        title: 'Regression Analysis',
        description: `Continuous prediction with ${column.uniqueValues} values`,
        confidence: 78,
        algorithms: ['Linear Regression', 'Random Forest'],
        reasoning: 'High variability suggests continuous target',
  ...
  // String columns
  else if (column.originalType === 'string') {
    if (column.uniqueValues === 2) {
      recommendations.push({
        icon: '⚡',
        title: 'Binary Classification',
        description: 'Two-class prediction',
        confidence: 90,
        algorithms: ['Logistic Regression', 'Random Forest'],
        reasoning: 'Two distinct categories',
        estimatedAccuracy: '80-90%',
        taskType: 'binary_classification'
      })
      ...
      recommendations.push({
        icon: '🎯',
        title: 'Multi-class Classification',
        description: `${column.uniqueValues} categories`,
        confidence: 78,
        algorithms: ['Random Forest', 'XGBoost'],
        reasoning: 'Good for categorical prediction',
        estimatedAccuracy: '70-80%',
        taskType: 'multiclass_classification'
      })
    }
  ...
  if (missingRatio > 0.15) {
    recommendations.push({
      ...
      description: `${Math.round(missingRatio * 100)}% missing data`,
      confidence: 50,
      reasoning: 'Missing values will impact performance',
      taskType: 'data_quality_issue'
    })
  }
...
const generateChart = async () => {
  ...
  try {
    ...
    if (selectedColumn.value.type === 'numerical' || selectedColumn.value.type === 'number') {
      const numbers = cleanData
...
const generateCategoricalChart = async (Chart, ctx, data) => {
  ...
  const entries = Object.entries(counts).sort(([,a], [,b]) => b - a).slice(0, 15)
...
const createHistogramChart = async (Chart, ctx, data) => {
  const bins = generateHistogramBins(data, Math.min(15, Math.max(5, Math.floor(Math.sqrt(data.length)))))
...
const createDoughnutChart = async (Chart, ctx, entries) => {
  chartInstance.value = new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: entries.map(([name]) => name.length > 20 ? name.substring(0, 20) + '...' : name),
      datasets: [{
...
const createBarChart = async (Chart, ctx, entries) => {
  chartInstance.value = new Chart(ctx, {
    type: 'bar',
    data: {
      labels: entries.map(([name]) => name.length > 15 ? name.substring(0, 15) + '...' : name),
      datasets: [{
        label: 'Count',
        data: entries.map(([, count]) => count),
        backgroundColor: 'rgba(102, 126, 234, 0.7)',
...
const getPieChartOptions = (title) => ({
  ...
  maintainAspectRatio: false,
  plugins: {
    legend: { position: 'right', labels: { color: 'rgba(255, 255, 255, 0.8)', padding: 15, usePointStyle: true } },
    title: { display: true, text: title, color: 'rgba(255, 255, 255, 0.9)' }
  }
})
...
const getColumnPreview = (columnName) => {
  ...
  const preview = columnData
    ...
    .map(val => String(val).length > 15 ? String(val).substring(0, 15) + '...' : String(val))
    .join(', ')
...
const getOutlierCount = () => {
  if (!selectedColumn.value || selectedColumn.value.type !== 'number') return 'N/A'
...
const getCompletenessPercentage = () => {
  return selectedColumn.value ? Math.round(100 - selectedColumn.value.missingPercent) : 0
}
...
const getSuitabilityFactors = () => {
  ...
  if (selectedColumn.value.missingPercent < 5) {
    ...
    factors.push({ icon: '⚠️', text: `${selectedColumn.value.missingPercent.toFixed(1)}% missing data`, status: 'warning' })
  }
...
onBeforeUnmount(() => {
  if (chartInstance.value) {
    console.log('💀 Component unmounting - destroying chart:', chartInstance.value.id)
    chartInstance.value.destroy()
    chartInstance.value = null
  }
})
...
onMounted(async () => {
  try {
    ...
    console.log('✅ Target Selection initialized:', {
      ...
      recommendedColumns: availableColumns.value.filter(col => col.recommended).length
    })
...
.helper-message, .helper-selected {
  display: flex;
  align-items: center;
  gap: 0.5rem;
}
